...
decryptFileBtn.addEventListener('click', async () => {
    ...
    try {
        ...
        downloadFileBtn.setAttribute('data-url', decryptedUrl);
        downloadFileBtn.setAttribute('data-filename', file.name.replace('encrypted_', ''));
...
downloadFileBtn.addEventListener('click', () => {
    const url = downloadFileBtn.getAttribute('data-url');
    const filename = downloadFileBtn.getAttribute('data-filename');
    ...
    return new Promise((resolve, reject) => {
        ...
        reader.onload = async (event) => {
            try {
                ...
                // Add a header to identify this as an encrypted Kavach file
                const header = new TextEncoder().encode("KAVACH_ENCRYPTED_FILE:");
    ...
    return new Promise((resolve, reject) => {
        ...
        reader.onload = async (event) => {
            try {
                ...
                // Check for header
                const expectedHeader = new TextEncoder().encode("KAVACH_ENCRYPTED_FILE:");
                
                // Check if file has the correct header
                let isKavachFile = true;
                ...
                if (!isKavachFile) {
                    throw new Error("Invalid file format or not a Kavach encrypted file");
                }
                ...
                // We need to guess the MIME type based on file extension
    ...
    if (fileInfo) {
        ...
        fetch(fileInfo.url)
            ...
            .then(blob => {
                ...
                alert(`File "${fileInfo.name}" is ready to be decrypted with the provided key.`);
            })
            .catch(error => {
